...
const App = () => {
  ...
  const handleRegenerateStory = async (originalStory, regeneratePrompt) => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/regenerate-story`, { story: originalStory, regeneratePrompt });
  ...
  const handleRegenerateImage = async (originalSummary, regeneratePrompt) => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/regenerate-image`, { summary: originalSummary, regeneratePrompt });
  ...
  const handleDescribeImage = async (imageUrl) => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/describe-image`, { imageUrl });
